...
def run_demo() -> None:
    ...
    print(f"tick#1 delivered={metrics1.delivered_events} slots={metrics1.total_slots} synapses={metrics1.total_synapses}")

    # Frame 2: move the bright pixel to exercise a different mapping
    frame[3][4] = 0.0
    frame[5][6] = 1.0
    metrics2 = region.tick_2d("pixels", frame)
    print(f"tick#2 delivered={metrics2.delivered_events} slots={metrics2.total_slots} synapses={metrics2.total_synapses}")
