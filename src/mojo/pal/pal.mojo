...
fn counter_rng(seed: Int, step: Int, draw_kind: Int, layer_index: Int, unit_index: Int, draw_index: Int) -> Float64:
    ...
    key = _mix64(key ^ UInt64(step))
    key = _mix64(key ^ UInt64(draw_kind))
    key = _mix64(key ^ UInt64(layer_index))
    key = _mix64(key ^ UInt64(unit_index))
    key = _mix64(key ^ UInt64(draw_index))
