...
fn connect_layers_topographic(
    ...
    var src = region.layers[source_layer_index]
    var dst = region.layers[destination_layer_index]
    var source_height = src.height; var source_width = src.width
    var dest_height = dst.height; var dest_width = dst.width
    ...
    var neuron_list = src.get_neurons()
    var neuron_index = 0
    while neuron_index < neuron_list.len:
        var source_row_index = neuron_index / source_width
        var source_col_index = neuron_index % source_width
        var synapse_index = 0
        var outgoing_synapses = neuron_list[neuron_index].outgoing
        while synapse_index < outgoing_synapses.len:
            var center_index = outgoing_synapses[synapse_index].target_index
            var center_row_index = center_index / dest_width
            var center_col_index = center_index % dest_width
            var delta_row = Float64(source_row_index - center_row_index)
            var delta_col = Float64(source_col_index - center_col_index)
            var squared_distance = delta_row*delta_row + delta_col*delta_col
            var weight_value: Float64 = 0.0
            if config.weight_mode == "dog":
                var weight_center = exp(-squared_distance / (2.0 * config.sigma_center * config.sigma_center))
                var weight_surround = exp(-squared_distance / (2.0 * config.sigma_surround * config.sigma_surround))
                var diff = weight_center - config.surround_ratio * weight_surround
    ...
    if config.normalize_incoming:
        ...
        while normalize_neuron_index < neuron_list.len:
            ...
            while normalize_synapse_index < synapses_for_scale.len:
                var center_index2 = synapses_for_scale[normalize_synapse_index].target_index
                var incoming_sum_value = incoming_sums[center_index2]
