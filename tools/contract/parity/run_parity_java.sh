...
bus_decay_cmd=(bash -lc "cd \"${repo_root}\" && mvn -q test -Dtest=ai.nektron.grownet.tests.LateralBusDecayTests")
windowed_cmd=(bash -lc "cd \"${repo_root}\" && mvn -q test -Dtest=ai.nektron.grownet.tests.WindowedWiringTests,ai.nektron.grownet.tests.EdgeEnumerationTests")
one_growth_cmd=(bash -lc "cd \"${repo_root}\" && mvn -q test -Dtest=ai.nektron.grownet.tests.RegionOneGrowthPerTickTests")
